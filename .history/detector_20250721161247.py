...
class OfficeDetector:
    ...
    def extract_features(self, file_path):
        ...
        try:
            ...
            print(f"  ERROR {file_path.name}: 特征提取失败 - {e}")
            return None
    ...
    def predict_file(self, file_path, features=None):
        ...
        for model_name, model in self.models.items():
            try:
                ...
                print(f"  ERROR {model_name} 预测失败: {e}")
                results[model_name] = {
                    'prediction': None,
                    'confidence': None,
                    'label': '错误'
    ...
    def predict_folder(self, folder_path, save_files=True, save_type='all'):
        ...
        if not all_files:
            print(f"ERROR 在 {folder_path} 中未找到文件")
            return
        ...
        for i, file_path in enumerate(all_files, 1):
            ...
            if results:
                ...
                consensus_label = f"[警告] {ensemble['label']}" if ensemble['label'] == '恶意' else f"[正常] {ensemble['label']}"
...
def save_files_by_type(files_list, file_type, save_files=True):
    """通用文件保存方法"""
    if not files_list:
        print(f"\n[文件夹] 未检测到{file_type}文件，无需保存")
        return

    if not save_files:
        print(f"\n[文件夹] 检测出 {len(files_list)} 个{file_type}文件 (已禁用保存功能)")
        return
    ...
    print(f"\n[文件夹] 正在保存 {len(files_list)} 个{file_type}文件到: {target_dir}")
    ...
    # 简洁的总结信息
    print(f"[完成] 保存完成: {saved_count} 个新文件, {skipped_count} 个已存在, {error_count} 个失败")
...
def print_test_summary(total_files, successful_predictions, model_stats, ensemble_stats):
    ...
    print(f"[文件夹] 总文件数: {total_files}")
    print(f"[成功] 成功分析: {successful_predictions}")
    print(f"[失败] 分析失败: {total_files - successful_predictions}")
