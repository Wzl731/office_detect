...
class OriginalVBAFeatureExtractor:
    ...
    def calculate_obfuscation_features(self, vba_code):
        ...
        # F8-F12: 每类在所有过程中的最大值 (按feature222.py的顺序)
        features.extend([
            max(concat_list),      # PROCEDURE_CONCAT_CNT
            max(paren_list),       # PROCEDURE_PARENTHESE_CNT
            max(ops_list),         # PROCEDURE_OPS_CNT
            max(assign_list),      # PROCEDURE_EQUAL_CNT
            max(str_list),         # PROCEDURE_STR_CNT
        ])
    ...
    def extract_features_from_file(self, file_path):
        """从单个文件提取完整的特征 - 兼容feature222.py的124维格式"""
