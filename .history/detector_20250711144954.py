...
class OfficeDetector:
    ...
    def predict_file(self, file_path, features=None):
        ...
        results['ensemble'] = {
            ...
            'total_votes': len([r for r in results.values() if r['prediction'] is not None]),
    ...
    def predict_folder(self, folder_path, save_files=True, save_type='all'):
        ...
        if save_type in ['all', 'malicious']:
            save_files_by_type(rf_malicious_files, '恶意', save_files)
        if save_type in ['all', 'benign']:
            save_files_by_type(rf_benign_files, '良性', save_files)
...
def main():
    ...
    parser.add_argument('--folder', '-f', default='good2bad', help='要测试的文件夹路径 (默认: good2bad)')
