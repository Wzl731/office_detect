...
class OriginalVBAFeatureExtractor:
    ...
    def get_feature_names(self):
        ...
        # 保留的VBA函数
        keep_vba_functions = [
            'Chr', 'Asc', 'Mid', 'Replace', 'Join', 'StrConv', 'Xor', 'Rnd', 'Val', 'Str', 'Space', 'Environ',
        ...
        keep_suspicious_keywords = [
            ...
            'AutoOpen', 'Document_Open', 'ExecuteExcel4Macro', 'CreateProcessA', 'Environ', 'ShowWindow'
        ]
        # 基础特征
        feature_names = [
            'FILENAME',
        ...
        feature_names += [
            'LINE_CONCAT_CNT',
            'LINE_STR_CNT',
            'PROCEDURE_CONCAT_CNT',      # 恢复 - 高级特征需要
            'PROCEDURE_PARENTHESE_CNT',  # 恢复 - 高级特征需要
            'PROCEDURE_OPS_CNT',
            'PROCEDURE_EQUAL_CNT',
            'PROCEDURE_STR_CNT',
        ]
    ...
    def calculate_obfuscation_features(self, vba_code):
        ...
        """
        # 计算实际需要的特征数量: 基础(11) + VBA函数(29) = 40个
        expected_features = 11 + 29  # 基础特征 + 筛选后的VBA函数
        if not vba_code:
