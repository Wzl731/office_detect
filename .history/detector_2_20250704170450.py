...
class VBAMalwareDetectorOriginal:
    def __init__(self, models_dir='models', use_post_processing=True):
        ...
        if self.use_post_processing:
            try:
                # 获取全局规则实例
                self.post_processor = get_post_processing_rules()
                if self.post_processor.is_available():
                    print("🧠 智能后处理规则已加载")
                    print(self.post_processor.get_rules_summary())
                else:
                    print("⚠️  后处理规则不可用")
...
使用示例:
  ...
  python detector.py data/good250623    # 测试data/good250623文件夹（位置参数）
  python detector.py --folder test_data # 测试test_data文件夹（选项参数）
  python detector.py -f my_samples     # 测试my_samples文件夹（选项参数）
        """
    )
    ...
    parser.add_argument(
        '--save-type',
        choices=['none', 'all', 'malicious', 'benign'],
        default='none',
        help='选择保存文件类型: none(不保存), all(全部), malicious(仅恶意), benign(仅良性) (默认: none)'
    )
    ...
    # 确定要使用的文件夹路径（优先使用 --folder 选项）
    folder_path = args.folder_option if args.folder_option else args.folder
    ...
    # 测试指定文件夹
    detector.test(folder_path, save_files=not args.no_save, save_type=args.save_type)
...
'''
# 默认行为：不保存文件
python detector.py -f data/bad250623
