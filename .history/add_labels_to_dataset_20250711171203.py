...
def process_ds1_and_ds2():
    ...
    if success1 and success2:
        ...
        try:
            # 使用 CSV 格式读取（更稳定）
            df1 = pd.read_csv('ds_date/ds1_labeled.csv')
            df2 = pd.read_csv('ds_date/ds2_labeled.csv')

            print(f"  ds1: {len(df1)} 条记录, {len(df1.columns)} 列")
            print(f"  ds2: {len(df2)} 条记录, {len(df2.columns)} 列")
            ...
            # 保存为多种格式
            combined_df.to_csv('ds_date/combined_dataset.csv', index=False)
            print(f"  💾 已保存到: ds_date/combined_dataset.csv")
            ...
            try:
                ...
                print("  ⚠️  无法保存为 xls 格式，仅保存 csv 格式")
