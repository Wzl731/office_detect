...
class VBAMalwareModelTrainer:
    def __init__(self, dataset_file='ds_date/combined_dataset.csv', models_dir=None, selected_models=None,
                 use_hyperparameter_tuning=False, search_method='grid'):
        """初始化模型训练器"""
        self.models = {}
        self.scalers = {}
        ...
        if models_dir is None:
            ...
            tuning_suffix = "_tuned" if use_hyperparameter_tuning else ""
            self.models_dir = Path(f"models_{models_suffix}{tuning_suffix}_{timestamp}")
        else:
    ...
    def load_dataset(self):
        ...
        # 检查各种可能的文件名列
        filename_candidates = ['filename', 'FILENAME', 'file', 'FILE', 'name', 'NAME']
        for col in self.dataset.columns:
            if col in filename_candidates or col.upper() in filename_candidates:
    ...
    def train_models(self):
        ...
        for model_name, config in self.model_configs.items():
            ...
            try:
                ...
                # 超参数优化
                if self.use_hyperparameter_tuning and model_name in self.hyperparameter_grids:
                    print(f"    🔍 开始超参数优化 ({self.search_method})...")
    ...
    def generate_detailed_report(self):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write(f"  - 良性样本: {self.benign_samples_cnt}\n")
            f.write(f"  - 恶意样本: {self.malicious_samples_cnt}\n")
...
def main():
    ...
    parser.add_argument('--tune', action='store_true',
                       help='启用超参数优化')
    parser.add_argument('--search-method', choices=['grid', 'random'], default='grid',
                       help='超参数搜索方法: grid(网格搜索) 或 random(随机搜索) (默认: grid)')
    ...
    tuning_info = f" (超参数优化: {args.search_method})" if args.tune else " (默认参数)"
    print(f"🚀 开始训练模型: {', '.join(selected_models)}{tuning_info}")

    trainer = VBAMalwareModelTrainer(
        args.dataset, args.models_dir, selected_models,
        args.tune, args.search_method
    )
    success = trainer.run()
