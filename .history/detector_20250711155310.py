...
class OfficeDetector:
    ...
    def predict_file(self, file_path, features=None):
        ...
        if features is None:
            ...
            print(f"  调试: 特征提取结果 = {features is not None}")
            if features is not None:
                print(f"  调试: 特征形状 = {features.shape}")
            if features is None:
                return None

        results = {}
        print(f"  调试: 模型数量 = {len(self.models)}")
        print(f"  调试: 模型名称 = {list(self.models.keys())}")

        for model_name, model in self.models.items():
            print(f"  调试: 开始预测 {model_name}")
            try:
                # 准备特征数据
                X = features.copy()
                ...
                print(f"  调试: {model_name} 预测成功: {prediction}")
    ...
    def predict_folder(self, folder_path, save_files=True, save_type='all'):
        ...
        for i, file_path in enumerate(all_files, 1):
            ...
            # 调试信息
            print(f"   调试: results is None = {results is None}")
            if results:
                print(f"   调试: results keys = {list(results.keys())}")
                for k, v in results.items():
                    if k != 'ensemble':
                        print(f"   调试: {k} prediction = {v['prediction']}")
