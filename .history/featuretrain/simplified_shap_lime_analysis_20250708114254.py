...
class SimplifiedSHAPLIMEAnalysis:
    ...
    def generate_comprehensive_report(self):
        """生成简化版SHAP-LIME联合分析报告"""
        print("\n📄 生成SHAP-LIME联合分析报告...")

        try:
            report_file = self.output_dir / 'simplified_shap_lime_analysis_report.md'

            with open(report_file, 'w', encoding='utf-8') as f:
                f.write("# 简化版SHAP-LIME联合分析优化策略报告\n\n")

                f.write("## 🎯 分析目标\n\n")
                f.write("本报告基于现有SHAP分析数据，结合模拟的LIME局部可解释性分析，")
                f.write("通过一致性验证为特征优化提供可靠的策略建议。\n\n")

                f.write("## 📊 方法论\n\n")
                f.write("### 数据来源\n")
                f.write("- **SHAP数据**: 使用feature_analysis/shap_pattern_analysis.xlsx中的现有分析结果\n")
                f.write("- **LIME模拟**: 基于SHAP重要性模拟局部可解释性，添加合理的局部变化\n")
                f.write("- **三维评分**: 结合稳定性、判别力、可解释性的综合评估\n\n")

                # 分析结果统计
                f.write("## 📈 分析结果统计\n\n")
                f.write(f"- **总特征数**: {len(self.lime_simulation_df)}\n")

                high_consistency = len(self.lime_simulation_df[self.lime_simulation_df['consistency_score'] > 0.8])
                medium_consistency = len(self.lime_simulation_df[(self.lime_simulation_df['consistency_score'] > 0.6) &
                                                               (self.lime_simulation_df['consistency_score'] <= 0.8)])
                low_consistency = len(self.lime_simulation_df[self.lime_simulation_df['consistency_score'] <= 0.6])

                f.write(f"- **高一致性特征** (>0.8): {high_consistency} ({high_consistency/len(self.lime_simulation_df)*100:.1f}%)\n")
                f.write(f"- **中等一致性特征** (0.6-0.8): {medium_consistency} ({medium_consistency/len(self.lime_simulation_df)*100:.1f}%)\n")
                f.write(f"- **低一致性特征** (<0.6): {low_consistency} ({low_consistency/len(self.lime_simulation_df)*100:.1f}%)\n\n")

                # 高可信度特征
                f.write("## 🏆 高联合可信度特征 (Top 10)\n\n")
                top_reliable = self.joint_strategy_df.head(10)
                for i, (_, row) in enumerate(top_reliable.iterrows()):
                    f.write(f"{i+1}. **`{row['feature']}`** (可信度: {row['joint_reliability']:.3f})\n")
                    f.write(f"   - SHAP重要性: {row['shap_importance']:.4f}\n")
                    f.write(f"   - LIME重要性: {row['lime_importance']:.4f}\n")
                    f.write(f"   - 一致性: {row['consistency_score']:.3f}\n")
                    f.write(f"   - 策略: {row['optimization_strategy']}\n\n")
                ...
                f.write("## 📁 生成文件\n\n")
                f.write("- `simplified_shap_lime_joint_strategy.csv`: 联合优化策略详细数据\n")
                f.write("- `simplified_shap_lime_analysis.png`: 联合分析可视化图表\n")
                f.write("- `simplified_shap_lime_analysis_report.md`: 本报告\n")
    ...
    def run_simplified_analysis(self):
        """运行简化版SHAP-LIME联合分析"""
        print("🎯 简化版SHAP-LIME联合分析")
        print("=" * 60)
        print("基于现有SHAP数据 + 模拟LIME分析")
        print("=" * 60)

        # 1. 加载现有数据
        if not self.load_existing_shap_data():
            return False

        # 2. 模拟LIME分析
        if not self.simulate_lime_analysis():
            return False
        ...
        print(f"🔍 基于现有数据的快速联合分析策略已生成")

        return True

def main():
    """主函数"""
    analyzer = SimplifiedSHAPLIMEAnalysis()
    success = analyzer.run_simplified_analysis()
