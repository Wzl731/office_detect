...
class OfficeDetector:
    ...
    def predict_folder(self, folder_path, save_files=True, save_type='all'):
        ...
        for i, file_path in enumerate(all_files, 1):
            ...
            # 预测文件
            prediction_result = self._predict_single_file(file_path, model_stats, ensemble_stats)
    ...
    def _predict_single_file(self, file_path, model_stats, ensemble_stats):
        """预测单个文件并更新统计信息"""
        try:
            # 调用核心预测方法
            ...
            if not results:
                ...
                return {'success': False, 'is_malicious': False}
            ...
            if valid_predictions > 0:
                ...
                ensemble_label = "恶意" if is_malicious else "良性"
                emoji = "🚨" if is_malicious else "✅"
                print(f"   集成结果: {emoji} {ensemble_label} ({malicious_votes}/{len(self.models)}票)")
                ...
                return {'success': True, 'is_malicious': is_malicious}
            else:
                print("   集成结果: ❌ 所有模型预测失败")
                ensemble_stats['errors'] += 1
                return {'success': False, 'is_malicious': False}
...
def main():
    ...
    parser.add_argument('--models-dir', '-m', default='models', help='模型文件夹路径 (默认: models)')
